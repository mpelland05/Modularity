...
opt.numPermutations = 10000;	%Number of permutations to carry to verify statistical significance. 
...
for ii = 1:length(SpecName),
	...
	for ll = 1:length(gpath),
	    ...
	    pipeline = psom_add_job(pipeline,strcat('CmpDicePath',num2str(ll),'Samp',num2str(kk),'Scales',num2str(ii)),'CompareDicePairedMedianT',files_in,files_out,opt,0);
