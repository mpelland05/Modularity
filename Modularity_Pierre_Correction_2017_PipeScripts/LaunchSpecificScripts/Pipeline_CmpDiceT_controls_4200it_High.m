...
SpecName ={'H1825'} ;% {'L36' 'H1825' 'All'};%Name to give to specific subscales
...
opt.numPermutations = 4200;	%Number of permutations to carry to verify statistical significance. 
...
gpath{1} = 'E:\TemporaryData\';


pip.opt.psom.path_logs = 'H:\TemporaryDataLogs\';   %<---------------log outputs
...
for ii = 1:length(SpecName),
	...
	for ll = 1:length(gpath),
        ...
        kk
	    CompareDicePairedT(files_in,files_out,opt);
...
% Start pipeline
%%%%%%
%pip.opt.mode = 'session';
%pipt.opt.max_queued = 1;
