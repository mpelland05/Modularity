function MapQValuesAtomWise(files_in,files_out),
%files_in
...
%Find q-values
[qtemp test] = niak_fdr(pvalues(:),files_in.type, files_in.q);   %find q-values for subset of voxels Idx which did not have a p-value of 0
qtemp = (qtemp).*(test);                    % Get opposite to obtain nice maps
...
hdr.file_name = strcat(files_out,'_signed_Q.mnc.gz');
niak_write_vol(hdr,vol)
