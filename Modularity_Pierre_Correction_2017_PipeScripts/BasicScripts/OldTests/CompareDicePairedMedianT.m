function CompareDicePairedMedianT(files_in,files_out,opt),
...
%%%%%%%%%%%%%%%%%%
tt = strsplit('CBxxx_Versus_SCxxx','_Versus_');
grpnam1 = tt{1};
grpnam2 = tt{end};
...
%%%%%%%%%%%%%%%%%%
ogrp1 = median(sort(Avg.(opt.scaleName).(strcat('Self_',grpnam1)),3)(:,:,1:end-1),3); %Original self(within) Dice
ogrp2 = median(Avg.(opt.scaleName).(strcat(grpnam1,'_vs_',grpnam2)),3); %Orginal between Dice
odiff = ogrp1 - ogrp2;

mo1 = median(ogrp1,2)'; 								%ok
mo2 = median(ogrp2,2)'; 								%ok
modiff = median(odiff,2)'./( std(odiff,0,2)'./sqrt(size(ogrp1,2)) );
...
for pe = 1:opt.numPermutations,
    ...
    tgrp1 = median(sort(trawgrp1,3)(:,:,1:end-1),3);%ok in size and numbers (participans 2 and 12)
    tgrp2 = median(trawgrp2,3);							% same as above
    
    tdiff = tgrp1-tgrp2;
    mtdiff = median(tdiff,2)./(std(tdiff,0,2)'./sqrt(size(ogrp1,2)) );								%ok size
...
% Saving output
%%%%%%%%%%%%%%%%%%%%%%%%%
Perm.(opt.contrast).pvalues = pvalues;
Perm.(opt.contrast).Average.(ctrst1) = mo1;
Perm.(opt.contrast).Average.(ctrst2) = mo2;
Perm.(opt.contrast).Average.Difference = modiff;
Perm.(opt.contrast).STD.(ctrst1) = sdo1;
Perm.(opt.contrast).STD.(ctrst2) = sdo2;
Perm.(opt.contrast).STD.Difference = sdodiff;
