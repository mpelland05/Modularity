function CompareDicePaired(files_in,files_out,opt),
...
%mstep = 1;
%save('/home/mpelland/Test_current.mat');
%%%%%%%%%%%%%%%%%%
...
for pe = 1:opt.numPermutations,
    ...
    tdiff = tgrp1-tgrp2;
    mtdiff = mean(tdiff,2);							%ok size

    %tpos = mtdiff.*(modiff > 0).*(mtdiff > 0);
    %tneg = mtdiff.*(modiff < 0).*(mtdiff < 0);
