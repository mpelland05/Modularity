function MeasureFalsePositivesT(files_in,files_out,opt)
...
for ff = 1:length(files_in),
	...
	load( strcat(fi,'Samp1',fs,'Permutated',fs,'Perm_avg_OutputCmpTopo_',opt.specname{ss},'_PairedT.mat') ); 
	%fn = fieldnames(Perm);
	...
	for ii = 1:opt.nsamps,
	    ...
	    load( strcat(fi,'Samp',num2str(ii),fs,'Permutated',fs,'Perm_avg_OutputCmpTopo_',opt.specname{ss},'_PairedT.mat') );
...
%	    end
   ...
   save('-mat7-binary',fo, 'Results','Histo','pvalues','mdiff','effsz','PartName', 'stdiff');
   clear Results;
end
end
