...
%Modified august 18, 2017, so that the comparison matrix is much lighter. 
...
clear atoms;clear mask;clear Atom;
...
for ss = 1:size(opt.scales,2),%scales
    ...
    for gg = 1:length(opt.groups),%groups
        ...
        for pp = 1:length(PartIdx),
            [gg pp]
            [hdr Target] = niak_read_vol( strcat(files_in.path,fs,pnames{PartIdx(pp)},fs,ScaleStr,fs,'brain_partition_',opt.pType,'_ind_',pnames{PartIdx(pp)},'_',ScaleStr,'.mnc.gz') );% 
        ...
        for pph = 1:size(Group{gg},2),%horizon of matrix
            for ppv = 1:size(Group{gg},2),  %Vertical of matri
                ...
                for uu = 1:length(uAtom),
                    ...
                    NetTarget = Group{gg}(uu, pph);
                    NetVol = Group{gg}(uu, ppv);
                    
                    tLoc = find( Group{gg}(:, pph) == NetTarget);
                    vLoc = find( Group{gg}(:, ppv) == NetVol);
                    
                    NumTarget = sum(AtmSiz(tLoc));
                    NumVol = sum(AtmSiz(vLoc));
                    
                    NumTV = sum( AtmSiz(intersect(tLoc,vLoc)));
