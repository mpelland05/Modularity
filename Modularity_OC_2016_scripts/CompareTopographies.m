function CompareTopographies(files_in,files_out),
...
%               not specifying it in the files_in.groups, there will not be
...
Results.Parameters.PartitionType = files_in.pType;
Results.Parameters.Filter = files_in.filter;
...
for ss = 1:size(files_in.scales,2),%scales
    ...
    for gg = 1:length(files_in.groups),%groups
        ...
        for pp = 1:length(PartIdx),
            [gg pp]
            [hdr Target] = niak_read_vol( strcat(files_in.path,fs,pnames{PartIdx(pp)},fs,ScaleStr,fs,'brain_partition_',files_in.pType,'_ind_',pnames{PartIdx(pp)},'_',ScaleStr,'.mnc.gz') );% 
    ...
    for gg = 1:length(files_in.groups),%groups
        ...
        Results.(ScaleStr).(strcat('Self_',files_in.groups{gg})) = zeros(length(Loc),size(Group{gg},2),size(Group{gg},2));
